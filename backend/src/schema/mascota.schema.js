...
const mascotaSchema = Joi.object({
  ...
  nota: Joi.number().min(1).max(10).required().messages({
    ...
    "number.max": "La nota no puede ser mayor a 10.",
  }),
  ...
  propietario: Joi.string()
    ...
    .optional()
    .messages({
      "string.pattern.base": "El propietario debe ser un ID válido de MongoDB.",
    }),
    notasAdicionales: Joi.string().optional().messages({
      "string.base": "El campo notas adicionales debe ser de tipo string.",
    }),
}).messages({
...
const mascotaIdSchema = Joi.object({
  id: Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base": "El ID debe ser un ID válido de MongoDB.",
    }),
});
